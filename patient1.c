...
/*select the index option entered by patient1*/
...
int phase1_2(){
	...
	if (getsock_check == -1) {
		perror("Patient1 error: getsockname");
		exit(1);
	}
	...
	{
		perror("Patient1 Error: while sending\n");
		exit(1);
	}
	...
	{
		perror("Patient1 Error: when receiving bytes\n");
		exit(1);
	}
...
int phase3(){
	...
	*/
	...
	{
		perror("Patient1 Error: recvfrom");
		exit(1);
	}
	buf[numbytes] = '\0';
